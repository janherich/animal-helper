...
| T11 | Administrator account takeover exposes all open cases                                                   | Critical      | no sign-up, individual allow-list, required-UV passkeys, short sessions, revocation, anomaly audit                                  | Medium                          |
| T12 | CSRF or malicious origin performs administrator action                                                  | High          | SameSite=Strict, exact Origin, reject Fetch Metadata cross-site/same-site, same-origin admin cookies, re-confirm send/export        | Low                             |
