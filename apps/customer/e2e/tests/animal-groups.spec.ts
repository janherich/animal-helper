...
test('replaces many cards with fewer sequentially without collapsing the area', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Voľne žijúce zvieratá', exact: true }).click()
  await page.getByRole('button', { name: 'Vtáky', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Dravce', exact: true })).toBeVisible()
  ...
  const frames = await page.evaluate(async () => {
    Array.from(document.querySelectorAll<HTMLButtonElement>('.customer-animal-groups__grid button'))
      .find(button => button.textContent?.trim() === 'Dravce')!
      .click()
  ...
  await expect(page.getByRole('button', { name: 'Orol', exact: true })).toHaveCount(0)
  await page.evaluate(() =>
    document
  ...
  await expect(page.locator('.customer-animal-groups__grid')).toHaveCount(1)
  await expect(page.getByRole('button', { name: 'Orol', exact: true })).toBeVisible()
...
test('opens the manual group fixture, selects a group and preserves it on return', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.customer-animal-groups__grid button')).toHaveCount(4)
  ...
  await expect(page.getByRole('option', { name: 'Mačka', exact: true })).toContainText('Domáce zvieratá')
  ...
  await expect(page.getByRole('button', { name: 'Mačka', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await search.fill('pes')
  await expect(page.getByRole('option')).toHaveCount(1)
  await page.getByRole('option', { name: 'Pes', exact: true }).click()
  await expect(search).toHaveValue('')
  await expect(page.getByRole('button', { name: 'Pes', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await search.fill('morca')
  await page.getByRole('option', { name: 'Morča', exact: true }).click()
  await expect(grid.locator('button').first()).toHaveText('Morča')
  await expect(grid.locator('button').first()).toHaveAttribute('aria-pressed', 'true')
  await page.getByRole('button', { name: 'Škrečok', exact: true }).click()
  await expect(grid.locator('button').first()).toHaveText('Morča')
  await search.fill('xyz')
  await expect(page.getByRole('listbox')).toContainText('V katalógu sa nenašla zhoda.')
  ...
  await page.getByRole('radio', { name: 'Iné hospodárske zviera' }).check()
  ...
  await page.getByRole('button', { name: 'Sliepka', exact: true }).click()
  ...
  await expect(page.getByRole('option')).toHaveCount(2)
  await search.press('Escape')
  await expect(page.getByRole('button', { name: 'Krava', exact: true })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Potvrdiť voľbu' })).toBeDisabled()
  await page.getByRole('radio', { name: 'Neviem identifikovať' }).check()
  await page.getByRole('button', { name: 'Potvrdiť voľbu' }).click()
  await expect(page.getByRole('heading', { name: 'Doplňte podrobnosti' })).toBeVisible()
  ...
  await page.setViewportSize({ width: 402, height: 874 })
  await page.getByRole('button', { name: 'Voľne žijúce zvieratá', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Vtáky', exact: true })).toBeVisible()
  await expect(page.locator('.customer-animal-groups__cards')).not.toHaveAttribute('style', /height:/)
  await expect(grid).toHaveCount(1)
  ...
  await page.getByRole('button', { name: 'Dravce', exact: true }).click()
  await page.getByRole('button', { name: 'Orol', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Potvrdiť voľbu' })).toBeVisible()
